...
public class DemoOBJ : MonoBehaviour
{
	...
	private IEnumerator LoadCube (CubeQuery query, int x, int y, int z)
	{
		...
		if (!UseEbo) {
			...
			long predl = _sw.ElapsedMilliseconds;
			DebugLog ("ardvark Starting ebo dl for: " + path);
			loader = new WWW (path + ".ebo");
			DebugLog ("ardvark After new WWW: " + (_sw.ElapsedMilliseconds - predl));
			yield return loader;
			DebugLog ("ardvark After yield ret WWW: " + (_sw.ElapsedMilliseconds - predl));

			if (!string.IsNullOrEmpty (loader.error)) {
				yield break;
			}
			buffer.eboBuffer = loader.bytes;
			DebugLog ("ardvark after assign bytes: " + (_sw.ElapsedMilliseconds - predl));
			mtllib = "model.mtl";
		}
